...
interface CreatedNoteEventMetadata {
  ticketId: number;
  actorSub: string;
  auditId?: string;
...
/**
 * An outer transaction (the requester ownership-checked mutation path) cannot
 * publish until its caller commits. Keep the dimension snapshot captured under
 * the ticket lock attached to that exact returned row so publishCreatedNote()
 * can emit the same reporting event after the outer commit.
 */
const pendingCreatedNoteEvents = new WeakMap<Note, CreatedNoteEventMetadata>();
...
export async function create(
  ...
  const write = async (tx: Prisma.TransactionClient) => {
    ...
    const data = {
        ...
        // Keep the repository boundary fail-closed even if a future schema
        // migration accidentally changes the database default.
    ...
    return {
      ...
      } satisfies CreatedNoteEventMetadata,
    };
  };
  ...
  // publish explicitly after its outer transaction succeeds. Retain the exact
  // audit identity and locked dimension snapshot for that deferred emission.
  pendingCreatedNoteEvents.set(result.note, result.event);
  if (!transaction) publishCreatedNote(ticketId, result.note, actorSub);
  return result.note;
}
...
export function publishCreatedNote(
  ...
  const event = pendingCreatedNoteEvents.get(note);
  pendingCreatedNoteEvents.delete(note);
  publish({
    type: 'note.added',
    ticketId: event?.ticketId ?? ticketId,
    note,
    actor: event?.actorSub ?? actorSub,
    auditId: event?.auditId,
    firstResponseRecorded: event?.firstResponseRecorded,
    metricContext: event?.metricContext,
  });
}
