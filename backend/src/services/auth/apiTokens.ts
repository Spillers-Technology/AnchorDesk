...
/**
 * Resolve a presented raw token to its (active) owning user, its capability
 * scope, and its own row id (needed to scope intake idempotency keys to this
 * specific credential), or null. Rejects revoked/expired tokens and inactive
 * users. Stamps lastUsedAt best-effort so a write failure never blocks auth.
 */
export async function resolve(
  token: string,
): Promise<{ user: User; scope: ApiTokenScope; tokenId: number } | null> {
  ...
  prisma.apiToken
    ...
    .catch(() => {});
  return { user: row.user, scope: row.scope, tokenId: row.id };
}
