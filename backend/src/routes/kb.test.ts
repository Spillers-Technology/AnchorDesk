  ...
  if (role) {
    app.addHook('onRequest', async (request) => {
      if (role === 'portal') {
        ...
        request.actorSub = 'requester:22 (portal)';
...
describe('KB search principal boundary', () => {
  it('routes a requester portal contract only to the portal-safe repository function', async () => {
  ...
  it('does not expose the portal contract to an anonymous request', async () => {
  ...
  it('never widens a requester principal to internal search', async () => {
    const app = await appFor('portal');
    try {
      const response = await app.inject({
        method: 'GET',
...
describe('KB portal article principal boundary', () => {
  it('allows a requester to read only through the portal-safe repository function', async () => {
    const app = await appFor('portal');
    mockedKb.getPublishedPortalBySlug.mockResolvedValue({
      id: 4,
      slug: 'reset-password',
      title: 'Reset password',
      bodyHtml: '<p>Reset it.</p>',
      category: 'Accounts',
      visibility: 'portal',
      published: true,
      author: 'Support',
    ...
    try {
      const response = await app.inject({
        method: 'GET',
        url: '/kb/portal/reset-password',
      });
      expect(response.statusCode).toBe(200);
      expect(mockedKb.getPublishedPortalBySlug).toHaveBeenCalledWith('reset-password');
    } finally {
      await app.close();
    }
  });

  it('rejects a requester search that drifts from the exact deflection contract', async () => {
    const app = await appFor('portal');
    try {
      const response = await app.inject({
        method: 'GET',
        url: '/kb/search?q=password&visibility=portal&limit=4',
      });
      expect(response.statusCode).toBe(401);
      expect(mockedKb.searchPublishedStaff).not.toHaveBeenCalled();
      expect(mockedKb.searchPublishedPortal).not.toHaveBeenCalled();
  ...
  it('rejects an anonymous article read before repository access', async () => {
    const app = await appFor();
    try {
      const response = await app.inject({
        method: 'GET',
      ...
      expect(response.statusCode).toBe(401);
      expect(mockedKb.getPublishedPortalBySlug).not.toHaveBeenCalled();
    } finally {
      await app.close();
    }
  });

  it('rejects requester article query flags outside the positive allowlist', async () => {
    const app = await appFor('portal');
    try {
      const response = await app.inject({
        method: 'GET',
        url: '/kb/portal/reset-password?preview=true',
      });
      expect(response.statusCode).toBe(401);
      expect(mockedKb.getPublishedPortalBySlug).not.toHaveBeenCalled();
    } finally {
  ...
  it('preserves staff access while retaining the portal visibility boundary', async () => {
    const app = await appFor('readonly');
    mockedKb.getPublishedPortalBySlug.mockResolvedValue({
      id: 4,
      slug: 'reset-password',
