...
- **Connecting MCP:** header-capable clients can point SSE at `/mcp/sse` with `Authorization: Bearer <token>` (see `.mcp.json`, which reads `${ANCHORDESK_TOKEN}`). OAuth-capable clients such as ChatGPT discover `/.well-known/oauth-protected-resource`, use the configured OIDC issuer as the authorization server, and return with an OIDC bearer token. The `/mcp/*` endpoints are *not* public — they require a valid PAT, OIDC bearer, or session. With `OIDC_DISABLED=true` (dev) every request is the dev admin, including MCP.
