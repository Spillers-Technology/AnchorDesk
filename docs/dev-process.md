...
| 1b | Rework of Unit 1 after operator review blocked it (#51): fingerprint-gated 2.8.x-only baseline adoption in `apply-schema.mjs`, frozen `schema-2.8.prisma`, real-install fixtures from the published 2.7.2/2.8.0/2.8.2 images, `verify-baseline-upgrade.mjs` wired into CI | Claude (direct) | **astra (gpt-6-astra), Sol/high, read-only** | 7 (2 High, 4 Medium, 1 Low) | 6 real (5 reproduced against real databases; 1 confirmed by inspection) | 0 | ~85k review | Review paid for itself. Both Highs were real and reproduced: an extra CHECK constraint (or a dropped trigger) was adopted as stock 2.8 because Prisma's diff models neither, and an adoption interrupted between `migrate resolve` creating the history table and recording the baseline made the *next* start skip the fingerprint entirely and fail deploy over a populated database. Also real: an allowlisted index name on another table passed (name-only matching); an enum-only database counted as "empty" and was written to before failing; the harness could not have caught row destruction (refusal fixtures had no rows) or varchar/enum/sequence changes; and the CI equality step discarded a failing generator's exit code via process substitution. Fixes: a second catalog check (CHECK constraints, triggers, index identity), corrected state detection (real emptiness incl. enums/sequences/views, interrupted-adoption recovery, failed-migration stop), seeded refusal fixtures plus a stronger snapshot, and file-based CI comparison. Non-`public` schema adoption is now refused explicitly rather than left untested (astra's finding 7, PLAUSIBLE — not reproduced). Harness 23 -> 51 checks; negative controls fail exactly the intended cases. |
