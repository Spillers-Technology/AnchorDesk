...
- **`capture-mobile-media.mjs`** — the **mobile verification matrix**
  ...
  **Companies**, **Network**, **Sync**, and the **Admin** dashboard, teams,
  ...
  Every Reports/TIME shot asserts that the document itself has no horizontal
  overflow; wide charts and tracks must scroll only inside their own container.
  Output lands in
...
3. **Run the captures** from the repo root:
   ...
   ```bash
   ANCHORDESK_CAPTURE_DEVICES=galaxy,fold-closed ANCHORDESK_CAPTURE_VIEWS=reports,reports-sla,time-day,time-sla \
