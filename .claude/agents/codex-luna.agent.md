...
codex exec -m gpt-5.6-luna -c model_reasoning_effort=low --approve-for-me \
  ...
  "<the task prompt, verbatim from what you were given, plus: 'Follow this repo's existing patterns and CLAUDE.md conventions — Fastify 5 + Prisma repositories on the backend, MUI + the shared ticketVocab on the web client. Keep string literals ASCII-only.'>"
```
